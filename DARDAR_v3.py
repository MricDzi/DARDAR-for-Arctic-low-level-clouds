...
Python 3 : A. Dziduch, on 17/09/2024 -- DARDAR-SOCP v3
...
vers="", Surface_Type = True, textFile=True):
    ...
    rangeLat    = np.array(list(range(int(latMin)*100,int((latMax+dLat)*100),int(dLat*100))),float)/100.
    
    dLong       = resol_long_deg
    nbLong      = int((longMax-longMin)/resol_long_deg)       
    rangeLong   = np.array(list(range(int(longMin)*100,int((longMax+dLat)*100),int(dLong*100))),float)/100.
    ...
    listFINALE = list(np.sort(listAllFiles2process)) 
    ...
    for nHDFfiles in range(len(listFINALE)):
        ...
        if process > 0.:
            ...
            if len(idx_latBornes[0]) != 0:  # Useless condition because spatial limits were evaluated before.
                ...
                idz1 = np.where(np.abs(alti-altiMax) == min(np.abs(alti-altiMax)))[0][0]
                idz2 = np.where(np.abs(alti-altiMin) == min(np.abs(alti-altiMin)))[0][0]
                    ...
                    for nx in range(len(lat)):  # Scans all latitudes
                        ...
                        for nz in range(len(alti)):  # Scans all altitudes                     
                            ...
                            if mask_cloud[nx,nz] == 3.:
                                ...
                                # 1 layer = 3 pixels                                
                                if np.nansum(mask_cloud_iceOnly[nx,mixTop:len(alti)]) > 3.\
                                or np.nansum(mask_cloud_iceAndSupercooled[nx,mixTop:len(alti)]) > 3.:                             
                                ...
                                elif (np.nansum(mask_cloud_iceOnly[nx,mixTop-15:mixTop])>3. and np.nansum(mask_cloud_iceOnly[nx,mixTop:len(alti)])>3.)\
                                    or (np.nansum(mask_cloud_iceOnly[nx,mixTop-15:mixTop])>3. and np.nansum(mask_cloud_iceAndSupercooled[nx,mixTop:len(alti)])>3.)\
                                    or (np.nansum(mask_cloud_iceAndSupercooled[nx,mixTop-15:mixTop])>3. and np.nansum(mask_cloud_iceOnly[nx,mixTop:len(alti)])>3.)\
                                    or (np.nansum(mask_cloud_iceAndSupercooled[nx,mixTop-15:mixTop])>3. and np.nansum(mask_cloud_iceAndSupercooled[nx,mixTop:len(alti)])>3.):
                                    mask_cloud_supercooledOnlyMPC[nx,nz:len(alti)] = mask_cloud_supercooledOnlyMPC[nx,nz:len(alti)] * 1.
                                    mask_cloud_supercooledOnlyALONE[nx,nz:len(alti)] = mask_cloud_supercooledOnlyALONE[nx,nz:len(alti)] * 0.
                                    break
                                    
                                # Liquid layer with ice layer or ice+liquid layer ABOVE
                                if np.nansum(mask_cloud_iceOnly[nx,mixTop-15:mixTop])>3. or np.nansum(mask_cloud_iceAndSupercooled[nx,mixTop-15:mixTop])>3.:                                     
                                    mask_cloud_supercooledOnlyMPC[nx,nz:len(alti)] = mask_cloud_supercooledOnlyMPC[nx,nz:len(alti)] * 1.
                                    mask_cloud_supercooledOnlyALONE[nx,nz:len(alti)] = mask_cloud_supercooledOnlyALONE[nx,nz:len(alti)] * 0.
                                    break
                                else:                                                                                                     
                    ...
                    for nx in range(len(lat)):
                        ...
                        for nz in range(len(alti)):                            
                            if mask_cloud[nx,nz] == 4. or mask_cloud[nx,nz] == 3.:
                                ...
                                if np.nansum(mask_cloud_iceOnly[nx,mixTop-5:mixTop]) <= 3.\
                                and np.nansum(mask_cloud_iceOnly[nx,mixTop:len(alti)]) > 0.:
                                ...
                                elif np.nansum(mask_cloud_iceOnly[nx,mixTop-5:mixTop]) > 3.\
                                and np.nansum(mask_cloud_iceOnly[nx,mixTop:len(alti)]) > 0.:
                ...
                if CloudAnalysis == True:
                    ...
                    for nx in range(len(lat)):
                        ...
                        if np.nansum(Allpix_in_cloud[nx,:]) >= 3:
                ...
                if not 'dardarPix_in_grid_Count' in locals():
                    dardarPix_in_grid_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                
                # GM masks
                if not 'pix_in_cloud_Count' in locals():
                    pix_in_cloud_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if not 'Allpix_in_cloud_Count' in locals():
                    Allpix_in_cloud_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if not 'Allpix_in_cloud_occur_Count' in locals():
                    Allpix_in_cloud_occur_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if not 'cloud_occurence_in_pix_Count' in locals():
                    cloud_occurence_in_pix_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                
                if AllScooled_mask==True and not 'allSupercooled_Count' in locals():
                    allSupercooled_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if AllScooled_mask==True and not 'allSupercooled2_Count' in locals():
                    allSupercooled2_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if IceAndSupercooled_mask==True and not 'iceAndSupercooled_Count' in locals():
                    iceAndSupercooled_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if ScooledOnly_mask==True and not 'supercooledOnly_Count' in locals():
                    supercooledOnly_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if ScooledOnlyMPC_mask==True and not 'supercooledOnlyMPC_Count' in locals():
                    supercooledOnlyMPC_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if ScooledOnlyMPC_mask==True and not 'supercooledOnlyALONE_Count' in locals():
                    supercooledOnlyALONE_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if IceOnly_mask==True and not 'iceOnly_Count' in locals():
                    iceOnly_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if IceContaining_cloud_1_mask==True and not 'iceContaining_cloud_1_Count' in locals():
                    iceContaining_cloud_1_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if IceContaining_cloud_2_mask==True and not 'iceContaining_cloud_2_Count' in locals():
                    iceContaining_cloud_2_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if liquidWarm_mask==True and not 'liqWarm_Count' in locals():
                    liqWarm_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if classif_mix_clouds == True :
                    if not 'mask_cloud_sc_ice_BelowAbove_Count' in locals():
                        mask_cloud_sc_ice_BelowAbove_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'mask_cloud_sc_ice_BelowOnly_Count' in locals():
                        mask_cloud_sc_ice_BelowOnly_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'mask_cloud_sc_ice_BelowAbove2_Count' in locals():
                        mask_cloud_sc_ice_BelowAbove2_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'mask_cloud_sc_ice_BelowOnly2_Count' in locals():
                        mask_cloud_sc_ice_BelowOnly2_Count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                
                # Thermodynamic variables
                if temperature==True and not 'tempe_sum' in locals():
                    tempe_sum = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if temperature_2m==True and not 'tempe2m_sum' in locals():
                    tempe2m_sum = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if iwc_val==True and not 'iwc_sum' in locals():
                    iwc_sum = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if ext_val==True and not 'ext_sum' in locals():
                    ext_sum = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)        
                if re_val==True and not 're_sum' in locals():
                    re_sum = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                if lidarRatio_val==True and not 'LR_sum' in locals():
                    LR_sum = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                
                # CB masks
                
                if CloudSatPrecip == True :
                    if not 'CSRain_count' in locals():
                        CSRain_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'CSSnow_count' in locals():
                        CSSnow_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'CSMixed_count' in locals():
                        CSMixed_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'CSAll_count' in locals():
                        CSAll_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                
                if Surface_Type == True : 
                    if not 'Surface_Land_count' in locals():
                        Surface_Land_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'Surface_Water_count' in locals():
                        Surface_Water_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'Surface_Ice_count' in locals():
                        Surface_Ice_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float) 
                        
                
                if CloudAnalysis == True :
                    if not 'Cloud_Between_Lidar_count' in locals():
                        Cloud_Between_Lidar_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'MPC_count' in locals():
                        MPC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'MPCTemp_count' in locals():
                        MPCTemp_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'USLC_count' in locals():
                        USLC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'USLC2_count' in locals():
                        USLC2_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'USLCTemp_count' in locals():
                        USLCTemp_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'USLC2Temp_count' in locals():
                        USLC2Temp_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'IC_count' in locals():
                        IC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'ICTemp_count' in locals():
                        ICTemp_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'WC_count' in locals():
                        WC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'CC_count' in locals():
                        CC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'CCMatrix_count' in locals():
                        CCMatrix_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'TotalClouds_count' in locals():
                        TotalClouds_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'TotalCloudsMatrix_count' in locals():
                        TotalCloudsMatrix_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'SingleClouds_count' in locals():
                        SingleClouds_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'MultiClouds_count' in locals():
                        MultiClouds_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                
                    if not 'MPCMatrix_count' in locals():
                        MPCMatrix_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'HeightMPC_count' in locals():
                        HeightMPC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'TempMPC_count' in locals():
                        TempMPC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'ThicknessMPC_count' in locals():
                        ThicknessMPC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                
                    if not 'USLCMatrix_count' in locals():
                        USLCMatrix_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'HeightUSLC_count' in locals():
                        HeightUSLC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'TempUSLC_count' in locals():
                        TempUSLC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'ThicknessUSLC_count' in locals():
                        ThicknessUSLC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                
                    if not 'USLC2Matrix_count' in locals():
                        USLC2Matrix_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'HeightUSLC2_count' in locals():
                        HeightUSLC2_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'TempUSLC2_count' in locals():
                        TempUSLC2_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'ThicknessUSLC2_count' in locals():
                        ThicknessUSLC2_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                
                    if not 'ICMatrix_count' in locals():
                        ICMatrix_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'HeightIC_count' in locals():
                        HeightIC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'TempIC_count' in locals():
                        TempIC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'ThicknessIC_count' in locals(): 
                        ThicknessIC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    
                    if not 'WCMatrix_count' in locals():
                        WCMatrix_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'HeightWC_count' in locals():
                        HeightWC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'TempWC_count' in locals():
                        TempWC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'ThicknessWC_count' in locals():
                        ThicknessWC_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    
                    if not 'Precip_count' in locals():
                        Precip_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'PrecipMatrix_count' in locals():
                        PrecipMatrix_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'ColdRain_count' in locals():
                        ColdRain_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'ColdRainMatrix_count' in locals():
                        ColdRainMatrix_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    
                    if not 'ColdPrecip_count' in locals():
                        ColdPrecip_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'ColdPrecipMatrix_count' in locals():
                        ColdPrecipMatrix_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'WarmPrecip_count' in locals():
                        WarmPrecip_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    if not 'WarmPrecipMatrix_count' in locals():
                        WarmPrecipMatrix_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    
                    if MeteoConditions==True :
                        if not 'geopotential_700_count' in locals():
                            geopotential_700_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'spec_hum_700_count' in locals():
                            spec_hum_700_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'temperature_700_count' in locals():
                            temperature_700_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'geopotential_850_count' in locals():
                            geopotential_850_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'spec_hum_850_count' in locals():
                            spec_hum_850_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'temperature_850_count' in locals():
                            temperature_850_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'SST_Skin_count' in locals():
                            SST_Skin_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'surf_temp_count' in locals():
                            surf_temp_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'surf_press_count' in locals():
                            surf_press_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        
                        if not 'SH500m_count' in locals():
                            SH500m_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'SH1000m_count' in locals():
                            SH1000m_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'SH1500m_count' in locals():
                            SH1500m_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'pix500_count' in locals():
                            pix500_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'pix1000_count' in locals():
                            pix1000_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'pix1500_count' in locals():
                            pix1500_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                    
                    if Winds==True:
                        if not 'u_winds_700_count' in locals():
                            u_winds_700_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'v_winds_700_count' in locals():
                            v_winds_700_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'u_winds_850_count' in locals():
                            u_winds_850_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'v_winds_850_count' in locals():
                            v_winds_850_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'Surface_Uwind_count' in locals():
                            Surface_Uwind_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'Surface_Vwind_count' in locals():
                            Surface_Vwind_count = np.array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                ...
                for nlat in range(len(rangeLat)-1):
                    for nlong in range(len(rangeLong)-1):
                        ...
                        idx_deltaLat = np.array(list(set((id1[0])).intersection(set((id2[0])))))
                        
                        
                        # Surface condition 
                                    
                        if len(idx_deltaLat) != 0:
                            if landWaterFlag != None:
                                if landWaterFlag == "water":
                        ...
                        if len(idx_deltaLat) != 0 and len(idx_deltaLat2) != 0:
                            
                            if Surface_Type == True: 
                                if not np.isnan(np.nansum(land_surface_mask[idx_deltaLat2])):
                                    Surface_Land_count[nlat,nlong] += np.nansum(land_surface_mask[idx_deltaLat2])
                                if not np.isnan(np.nansum(water_surface_mask[idx_deltaLat2])):
                                    Surface_Water_count[nlat,nlong] += np.nansum(water_surface_mask[idx_deltaLat2])
                                if not np.isnan(np.nansum(ice_surface_mask[idx_deltaLat2])):
                                    Surface_Ice_count[nlat,nlong] += np.nansum(ice_surface_mask[idx_deltaLat2])

                            dardarPix_in_grid_Count[nlat,nlong] += float(len(idx_deltaLat2))                                                                                 
                            pix_in_cloud_Count[nlat,nlong] += float(np.nansum(pix_in_cloud[idx_deltaLat2,:]))
                            
                            # For GM only
                            chk = np.copy(np.nansum(pix_in_cloud[idx_deltaLat2,:],1))
                            chk[(chk<3)] = 0.
                            chk[(chk>=3)]  = 1.
                            cloud_occurence_in_pix_Count[nlat,nlong] += np.nansum(chk)                                                      
                            del chk
                            # For GM only
                            Allpix_in_cloud_Count[nlat,nlong] += float(np.nansum(Allpix_in_cloud[idx_deltaLat2,:]))
                            # For GM only
                            chk = np.copy(np.nansum(Allpix_in_cloud[idx_deltaLat2,:],1))
                            chk[(chk<3)] = 0.
                            chk[(chk>=3)]  = 1.
                            Allpix_in_cloud_occur_Count[nlat,nlong] += np.nansum(chk)
                            del chk
                            
                            # IceAndSC_GEO file
                            if IceAndSupercooled_mask == True:                           
                                if float(np.nansum(mask_cloud_iceAndSupercooled[idx_deltaLat2,:])) != 0.:                                        
                                    chk = np.copy(np.nansum(mask_cloud_iceAndSupercooled[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0.
                                    chk[(chk>=3)] = 1.
                                    iceAndSupercooled_Count[nlat,nlong] += np.nansum(chk)
                                    del chk                                  
                                else:
                            ...
                            if AllScooled_mask == True:
                                #AllSCGEO file
                                if float(np.nansum(mask_cloud_allSupercooled[idx_deltaLat2,:])) != 0.:                                        
                                    chk = np.copy(np.nansum(mask_cloud_allSupercooled[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0.
                                    chk[(chk>=3)] = 1.
                                    allSupercooled_Count[nlat,nlong] += np.nansum(chk)
                                    del chk                                  
                                else:
                                    allSupercooled_Count[nlat,nlong] += 0.                                
                                #AllSC2GEO file
                                if float(np.nansum(mask_cloud_allSupercooled2[idx_deltaLat2,:])) != 0.:                                        
                                    chk=np.copy(np.nansum(mask_cloud_allSupercooled2[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0.
                                    chk[(chk>=3)] = 1.
                                    allSupercooled2_Count[nlat,nlong] += np.nansum(chk)
                                    del chk                                  
                                else:
                            ...
                            #SCOnlyGEO file
                            if ScooledOnly_mask == True:  
                                if float(np.nansum(mask_cloud_supercooledOnly[idx_deltaLat2,:])) != 0.:                                        
                                    chk=np.copy(np.nansum(mask_cloud_supercooledOnly[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0.
                                    chk[(chk>=3)] = 1.
                                    supercooledOnly_Count[nlat,nlong] += np.nansum(chk)
                                    del chk                                  
                                else:
                            ...
                            if ScooledOnlyMPC_mask == True:
                                #SCOnlyMPCGEO file
                                if float(np.nansum(mask_cloud_supercooledOnlyMPC[idx_deltaLat2,:])) != 0.:                                        
                                    chk=np.copy(np.nansum(mask_cloud_supercooledOnlyMPC[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0.
                                    chk[(chk>=3)] = 1.
                                    supercooledOnlyMPC_Count[nlat,nlong] += np.nansum(chk)
                                    del chk                                  
                                else:
                                    supercooledOnlyMPC_Count[nlat,nlong] += 0.
                                #SCOnlyALONEGEO file
                                if float(np.nansum(mask_cloud_supercooledOnlyALONE[idx_deltaLat2,:])) != 0.:                                        
                                    chk=np.copy(np.nansum(mask_cloud_supercooledOnlyALONE[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0.
                                    chk[(chk>=3)] = 1.
                                    supercooledOnlyALONE_Count[nlat,nlong] += np.nansum(chk)
                                    del chk                                  
                                else:
                            ...
                            #IceOnlyGEO file
                            if IceOnly_mask == True:  
                                if float(np.nansum(mask_cloud_iceOnly[idx_deltaLat2,:])) != 0.:                                        
                                    chk=np.copy(np.nansum(mask_cloud_iceOnly[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0.
                                    chk[(chk>=3)] = 1.
                                    iceOnly_Count[nlat,nlong] += np.nansum(chk)
                                    del chk                                  
                                else:
                            ...
                            if IceContaining_cloud_1_mask == True:  
                                if float(np.nansum(mask_ice_containing_cloud_1[idx_deltaLat2,:])) != 0.:                                        
                                    chk = np.copy(np.nansum(mask_ice_containing_cloud_1[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0.
                                    chk[(chk>=3)] = 1.
                                    iceContaining_cloud_1_Count[nlat,nlong] += np.nansum(chk)
                                    del chk                                  
                                else:
                            ...
                            if IceContaining_cloud_2_mask == True:  
                                if float(np.nansum(mask_ice_containing_cloud_2[idx_deltaLat2,:])) !=0.:                                        
                                    chk = np.copy(np.nansum(mask_ice_containing_cloud_2[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0.
                                    chk[(chk>=3)] = 1.
                                    iceContaining_cloud_2_Count[nlat,nlong] += np.nansum(chk)
                                    del chk                                  
                                else:
                                    iceContaining_cloud_1_Count[nlat,nlong] += 0.
                            ...
                            #LiquidWarmGEO file
                            if liquidWarm_mask == True:  
                                if float(np.nansum(mask_cloud_liqWarm[idx_deltaLat2,:])) !=0.:                                        
                                    chk=np.copy(np.nansum(mask_cloud_liqWarm[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0.
                                    chk[(chk>=3)] = 1.
                                    liqWarm_Count[nlat,nlong] += np.nansum(chk)
                                    del chk                                  
                                else:
                            ...
                            if classif_mix_clouds == True :
                                ...
                                if float(np.nansum(mask_cloud_sc_ice_BelowAbove[idx_deltaLat2,:])) != 0.:                                        
                                    chk=np.copy(np.nansum(mask_cloud_sc_ice_BelowAbove[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0
                                    chk[(chk>=3)] = 1
                                    mask_cloud_sc_ice_BelowAbove_Count[nlat,nlong] += np.nansum(chk)
                                    del chk                                  
                                else:
                                    mask_cloud_sc_ice_BelowAbove_Count[nlat,nlong] += 0.
                                #SC_ice_belowOnlyGEO file
                                if float(np.nansum(mask_cloud_sc_ice_BelowOnly[idx_deltaLat2,:])) != 0.:                                        
                                    chk=np.copy(np.nansum(mask_cloud_sc_ice_BelowOnly[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0
                                    chk[(chk>=3)] = 1
                                    mask_cloud_sc_ice_BelowOnly_Count[nlat,nlong] += np.nansum(chk)                                  
                                    del chk
                                else:
                                    mask_cloud_sc_ice_BelowOnly_Count[nlat,nlong] += 0.
                                #SC_ice_belowAbove2GEO file
                                if float(np.nansum(mask_cloud_sc_ice_BelowAbove2[idx_deltaLat2,:])) != 0.:                                        
                                    chk=np.copy(np.nansum(mask_cloud_sc_ice_BelowAbove2[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0
                                    chk[(chk>=3)] = 1
                                    mask_cloud_sc_ice_BelowAbove2_Count[nlat,nlong] += np.nansum(chk)
                                    del chk                                  
                                else:
                                    mask_cloud_sc_ice_BelowAbove2_Count[nlat,nlong] += 0.
                                #SC_ice_belowOnly2GEO file
                                if float(np.nansum(mask_cloud_sc_ice_BelowOnly2[idx_deltaLat2,:])) != 0.:                                        
                                    chk=np.copy(np.nansum(mask_cloud_sc_ice_BelowOnly2[idx_deltaLat2,:],1))
                                    chk[(chk<3)] = 0
                                    chk[(chk>=3)] = 1
                                    mask_cloud_sc_ice_BelowOnly2_Count[nlat,nlong] += np.nansum(chk)                                  
                                    del chk
                                else:
                                    mask_cloud_sc_ice_BelowOnly2_Count[nlat,nlong] += 0.
                            
                            # Physical parameters
                            if temperature == True and (not np.isnan(np.nanmean(np.nanmean(tempe[idx_deltaLat2,:])))):            
                                tempe_sum[nlat,nlong] += np.nansum(tempe[idx_deltaLat2,:])
                            if temperature_2m == True and (not np.isnan(np.nanmean(tempe2m[idx_deltaLat2]))):
                                tempe2m_sum[nlat,nlong] += np.nansum(tempe2m[idx_deltaLat2])
                            if iwc_val == True and (not np.isnan(np.nanmean(np.nanmean(iwc[idx_deltaLat2,:])*1000.))):
                                iwc_sum[nlat,nlong] += np.nansum(iwc[idx_deltaLat2,:])*1000.     ## kg/m3 --> g/m3
                            if ext_val == True and (not np.isnan(np.nanmean(np.nanmean(ext[idx_deltaLat2,:])*1000.))):
                                ext_sum[nlat,nlong] += np.nansum(ext[idx_deltaLat2,:])*1000.     ## m-1 --> km-1
                            if re_val == True and (not np.isnan(np.nanmean(np.nanmean(re[idx_deltaLat2,:])*1000000.))):
                                re_sum[nlat,nlong] += np.nansum(re[idx_deltaLat2,:])*1000000.   ## m --> micrometers:           
                            if lidarRatio_val == True and (not np.isnan(np.nanmean(np.nanmean(LR[idx_deltaLat2,:])))):
                                LR_sum[nlat,nlong] += np.nansum(LR[idx_deltaLat2,:])
                            
                            # CloudSat Precip
                            if CloudSatPrecip == True :
                                if not np.isnan(np.nansum(CSRain[idx_deltaLat2])):
                                    CSRain_count[nlat,nlong] += np.nansum(CSRain[idx_deltaLat2])
                                if not np.isnan(np.nansum(CSSnow[idx_deltaLat2])):
                                    CSSnow_count[nlat,nlong] += np.nansum(CSSnow[idx_deltaLat2])
                                if not np.isnan(np.nansum(CSMixed[idx_deltaLat2])):
                                    CSMixed_count[nlat,nlong] += np.nansum(CSMixed[idx_deltaLat2])
                                if not np.isnan(np.nansum(CSAll[idx_deltaLat2])):
                                    CSAll_count[nlat,nlong] += np.nansum(CSAll[idx_deltaLat2])
                            
                        
                            # CB analysis
                            if CloudAnalysis == True:
                                if not np.isnan(np.nansum(LidarExtCloudMatrix[idx_deltaLat2])):
                                    Cloud_Between_Lidar_count[nlat,nlong] += np.nansum(LidarExtCloudMatrix[idx_deltaLat2])
                                if not np.isnan(np.nansum(MPC[idx_deltaLat2])):
                                    MPC_count[nlat,nlong] += np.nansum(MPC[idx_deltaLat2])
                                if not np.isnan(np.nansum(MPCTemp[idx_deltaLat2])):
                                    MPCTemp_count[nlat,nlong] += np.nansum(MPCTemp[idx_deltaLat2])
                                if not np.isnan(np.nansum(USLC[idx_deltaLat2])):
                                    USLC_count[nlat,nlong] += np.nansum(USLC[idx_deltaLat2])
                                if not np.isnan(np.nansum(USLC2[idx_deltaLat2])):
                                    USLC2_count[nlat,nlong] += np.nansum(USLC2[idx_deltaLat2])
                                if not np.isnan(np.nansum(USLCTemp[idx_deltaLat2])):
                                    USLCTemp_count[nlat,nlong] += np.nansum(USLCTemp[idx_deltaLat2])
                                if not np.isnan(np.nansum(USLC2Temp[idx_deltaLat2])):
                                    USLC2Temp_count[nlat,nlong] += np.nansum(USLC2Temp[idx_deltaLat2])
                                if not np.isnan(np.nansum(IceCloud[idx_deltaLat2])):
                                    IC_count[nlat,nlong] += np.nansum(IceCloud[idx_deltaLat2])
                                if not np.isnan(np.nansum(IceCloudTemp[idx_deltaLat2])):
                                    ICTemp_count[nlat,nlong] += np.nansum(IceCloudTemp[idx_deltaLat2])
                                if not np.isnan(np.nansum(WarmCloud[idx_deltaLat2])):
                                    WC_count[nlat,nlong] += np.nansum(WarmCloud[idx_deltaLat2])
                                if not np.isnan(np.nansum(ColdCloud[idx_deltaLat2])):
                                    CC_count[nlat,nlong] += np.nansum(ColdCloud[idx_deltaLat2])
                                if not np.isnan(np.nansum(ColdCloudMatrix[idx_deltaLat2])):
                                    CCMatrix_count[nlat,nlong] += np.nansum(ColdCloudMatrix[idx_deltaLat2])
                                if not np.isnan(np.nansum(TotalClouds[idx_deltaLat2])):
                                    TotalClouds_count[nlat,nlong] += np.nansum(TotalClouds[idx_deltaLat2])
                                if not np.isnan(np.nansum(TotalCloudsMatrix[idx_deltaLat2])):
                                    TotalCloudsMatrix_count[nlat,nlong] += np.nansum(TotalCloudsMatrix[idx_deltaLat2])
                                if not np.isnan(np.nansum(SingleClouds[idx_deltaLat2])):
                                    SingleClouds_count[nlat,nlong] += np.nansum(SingleClouds[idx_deltaLat2])
                                if not np.isnan(np.nansum(MultiClouds[idx_deltaLat2])):
                                    MultiClouds_count[nlat,nlong] += np.nansum(MultiClouds[idx_deltaLat2])
                                
                                if not np.isnan(np.nansum(MPCMatrix[idx_deltaLat2])):
                                    MPCMatrix_count[nlat,nlong] += np.nansum(MPCMatrix[idx_deltaLat2])
                                if not np.isnan(np.nansum(TopHeightMPC[idx_deltaLat2])):
                                    HeightMPC_count[nlat,nlong] += np.nansum(TopHeightMPC[idx_deltaLat2])
                                if not np.isnan(np.nansum(TopTempMPC[idx_deltaLat2])):
                                    TempMPC_count[nlat,nlong] += np.nansum(TopTempMPC[idx_deltaLat2])
                                if not np.isnan(np.nansum(ThicknessMPC[idx_deltaLat2])):
                                    ThicknessMPC_count[nlat,nlong] += np.nansum(ThicknessMPC[idx_deltaLat2])
                                
                                if not np.isnan(np.nansum(USLCMatrix[idx_deltaLat2])):
                                    USLCMatrix_count[nlat,nlong] += np.nansum(USLCMatrix[idx_deltaLat2])
                                if not np.isnan(np.nansum(TopHeightUSLC[idx_deltaLat2])):
                                    HeightUSLC_count[nlat,nlong] += np.nansum(TopHeightUSLC[idx_deltaLat2])
                                if not np.isnan(np.nansum(TopTempUSLC[idx_deltaLat2])):
                                    TempUSLC_count[nlat,nlong] += np.nansum(TopTempUSLC[idx_deltaLat2])
                                if not np.isnan(np.nansum(ThicknessUSLC[idx_deltaLat2])):
                                    ThicknessUSLC_count[nlat,nlong] += np.nansum(ThicknessUSLC[idx_deltaLat2])
                                    
                                if not np.isnan(np.nansum(USLC2Matrix[idx_deltaLat2])):
                                    USLC2Matrix_count[nlat,nlong] += np.nansum(USLC2Matrix[idx_deltaLat2])
                                if not np.isnan(np.nansum(TopHeightUSLC[idx_deltaLat2])):
                                    HeightUSLC2_count[nlat,nlong] += np.nansum(TopHeightUSLC2[idx_deltaLat2])
                                if not np.isnan(np.nansum(TopTempUSLC2[idx_deltaLat2])):
                                    TempUSLC2_count[nlat,nlong] += np.nansum(TopTempUSLC2[idx_deltaLat2])
                                if not np.isnan(np.nansum(ThicknessUSLC2[idx_deltaLat2])):
                                    ThicknessUSLC2_count[nlat,nlong] += np.nansum(ThicknessUSLC2[idx_deltaLat2])
                                
                                if not np.isnan(np.nansum(IceCloudMatrix[idx_deltaLat2])):
                                    ICMatrix_count[nlat,nlong] += np.nansum(IceCloudMatrix[idx_deltaLat2])
                                if not np.isnan(np.nansum(TopHeightIceCloud[idx_deltaLat2])):
                                    HeightIC_count[nlat,nlong] += np.nansum(TopHeightIceCloud[idx_deltaLat2])
                                if not np.isnan(np.nansum(TopTempIceCloud[idx_deltaLat2])):
                                    TempIC_count[nlat,nlong] += np.nansum(TopTempIceCloud[idx_deltaLat2])
                                if not np.isnan(np.nansum(ThicknessIceCloud[idx_deltaLat2])):
                                    ThicknessIC_count[nlat,nlong] += np.nansum(ThicknessIceCloud[idx_deltaLat2])
                                
                                if not np.isnan(np.nansum(WarmCloudMatrix[idx_deltaLat2])):
                                    WCMatrix_count[nlat,nlong] += np.nansum(WarmCloudMatrix[idx_deltaLat2])
                                if not np.isnan(np.nansum(TopHeightWarmCloud[idx_deltaLat2])):
                                    HeightWC_count[nlat,nlong] += np.nansum(TopHeightWarmCloud[idx_deltaLat2])
                                if not np.isnan(np.nansum(TopTempWarmCloud[idx_deltaLat2])):
                                    TempWC_count[nlat,nlong] += np.nansum(TopTempWarmCloud[idx_deltaLat2])
                                if not np.isnan(np.nansum(ThicknessWarmCloud[idx_deltaLat2])):
                                    ThicknessWC_count[nlat,nlong] += np.nansum(ThicknessWarmCloud[idx_deltaLat2])
                                
                                # DARDAR Precip
                                if not np.isnan(np.nansum(Precip[idx_deltaLat2])):
                                    Precip_count[nlat,nlong] += np.nansum(Precip[idx_deltaLat2])
                                if not np.isnan(np.nansum(PrecipMatrix[idx_deltaLat2])):
                                    PrecipMatrix_count[nlat,nlong] += np.nansum(PrecipMatrix[idx_deltaLat2])
                                if not np.isnan(np.nansum(ColdRain[idx_deltaLat2])):
                                    ColdRain_count[nlat,nlong] += np.nansum(ColdRain[idx_deltaLat2])
                                if not np.isnan(np.nansum(ColdRainMatrix[idx_deltaLat2])):
                                    ColdRainMatrix_count[nlat,nlong] += np.nansum(ColdRainMatrix[idx_deltaLat2])
                                if not np.isnan(np.nansum(ColdPrecip[idx_deltaLat2])):
                                    ColdPrecip_count[nlat,nlong] += np.nansum(ColdPrecip[idx_deltaLat2])
                                if not np.isnan(np.nansum(ColdPrecipMatrix[idx_deltaLat2])):
                                    ColdPrecipMatrix_count[nlat,nlong] += np.nansum(ColdPrecipMatrix[idx_deltaLat2])
                                if not np.isnan(np.nansum(WarmPrecip[idx_deltaLat2])):
                                    WarmPrecip_count[nlat,nlong] += np.nansum(WarmPrecip[idx_deltaLat2])
                                if not np.isnan(np.nansum(WarmPrecipMatrix[idx_deltaLat2])):
                                    WarmPrecipMatrix_count[nlat,nlong] += np.nansum(WarmPrecipMatrix[idx_deltaLat2])
                                    
                            if MeteoConditions == True:
                                if not np.isnan(np.nansum(geopotential_700[idx_deltaLat2])):
                                    geopotential_700_count[nlat,nlong] += np.nansum(geopotential_700[idx_deltaLat2])
                                if not np.isnan(np.nansum(spec_hum_700[idx_deltaLat2])):
                                    spec_hum_700_count[nlat,nlong] += np.nansum(spec_hum_700[idx_deltaLat2])
                                if not np.isnan(np.nansum(temperature_700[idx_deltaLat2])):
                                    temperature_700_count[nlat,nlong] += np.nansum(temperature_700[idx_deltaLat2])
                                if not np.isnan(np.nansum(geopotential_850[idx_deltaLat2])):
                                    geopotential_850_count[nlat,nlong] += np.nansum(geopotential_850[idx_deltaLat2])
                                if not np.isnan(np.nansum(spec_hum_850[idx_deltaLat2])):
                                    spec_hum_850_count[nlat,nlong] += np.nansum(spec_hum_850[idx_deltaLat2])
                                if not np.isnan(np.nansum(temperature_850[idx_deltaLat2])):
                                    temperature_850_count[nlat,nlong] += np.nansum(temperature_850[idx_deltaLat2])
                                if not np.isnan(np.nansum(SST_Skin_mask[idx_deltaLat2])):
                                    SST_Skin_count[nlat,nlong] += np.nansum(SST_Skin_mask[idx_deltaLat2])
                                if not np.isnan(np.nansum(surf_temp[idx_deltaLat2])):
                                    surf_temp_count[nlat,nlong] += np.nansum(surf_temp[idx_deltaLat2])
                                if not np.isnan(np.nansum(surf_press_mask[idx_deltaLat2])):
                                    surf_press_count[nlat,nlong] += np.nansum(surf_press_mask[idx_deltaLat2])
                                
                                # Not used
                                if not np.isnan(np.nansum(SH500m[idx_deltaLat2])):
                                    SH500m_count[nlat,nlong] += np.nansum(SH500m[idx_deltaLat2])
                                if not np.isnan(np.nansum(SH1000m[idx_deltaLat2])):
                                    SH1000m_count[nlat,nlong] += np.nansum(SH1000m[idx_deltaLat2])
                                if not np.isnan(np.nansum(SH1500m[idx_deltaLat2])):
                                    SH1500m_count[nlat,nlong] += np.nansum(SH1500m[idx_deltaLat2])
                                if not np.isnan(np.nansum(pix500[idx_deltaLat2])):
                                    pix500_count[nlat,nlong] += np.nansum(pix500[idx_deltaLat2])
                                if not np.isnan(np.nansum(pix1000[idx_deltaLat2])):
                                    pix1000_count[nlat,nlong] += np.nansum(pix1000[idx_deltaLat2])
                                if not np.isnan(np.nansum(pix1500[idx_deltaLat2])):
                                    pix1500_count[nlat,nlong] += np.nansum(pix1500[idx_deltaLat2])
                                    
                            if Winds == True:
                                if not np.isnan(np.nansum(u_winds_700[idx_deltaLat2])):
                                    u_winds_700_count[nlat,nlong] += np.nansum(u_winds_700[idx_deltaLat2])
                                if not np.isnan(np.nansum(v_winds_700[idx_deltaLat2])):
                                    v_winds_700_count[nlat,nlong] += np.nansum(v_winds_700[idx_deltaLat2])
                                if not np.isnan(np.nansum(u_winds_850[idx_deltaLat2])):
                                    u_winds_850_count[nlat,nlong] += np.nansum(u_winds_850[idx_deltaLat2])
                                if not np.isnan(np.nansum(v_winds_850[idx_deltaLat2])):
                                    v_winds_850_count[nlat,nlong] += np.nansum(v_winds_850[idx_deltaLat2])
                                if not np.isnan(np.nansum(Surface_Uwind[idx_deltaLat2])):
                                    Surface_Uwind_count[nlat,nlong] += np.nansum(Surface_Uwind[idx_deltaLat2])
                                if not np.isnan(np.nansum(Surface_Vwind[idx_deltaLat2])):
                                    Surface_Vwind_count[nlat,nlong] += np.nansum(Surface_Vwind[idx_deltaLat2])
...
"""
# Parameters file
from paramFile_GeoProf_COVER_SO_v12\
