...
# from scipy.stats import nanmean, nanmedian, nanstd  # for remote calculation only
from scipy import nanmean, nanmedian, nanstd  # for local calculation
...
os.chdir("C:/Users/bazantay/Desktop/AYMERIC")
...
CloudAnalysis=True, MeteoConditions=True, Winds=True, coupling=True,\
CloudSatPrecip=True,\
vers="", textFile=True):
    ...
    # Additional prints
    print len(listAllFiles2process), "hdf DARDAR_CLOUD files to process."
    print "...PROCESSING..."
    ...
    for nHDFfiles in range(len(listFINALE)):
        ...
        if process > 0.:
            ...
            if idx_latBornes[0] != []:  # Useless condition because spatial limits were evaluated before.
                ...
                if CloudAnalysis == True:
                    ...
                    BottomTempMPC = np.zeros(len(lat))
                    BottomSHMPC = np.zeros(len(lat))
                    BottomPressMPC = np.zeros(len(lat))
                    
                    USLCMatrix = np.zeros(len(lat))
                    TopHeightUSLC = np.zeros(len(lat))
                    TopTempUSLC = np.zeros(len(lat))
                    ThicknessUSLC = np.zeros(len(lat))
                    BottomTempUSLC = np.zeros(len(lat))
                    BottomSHUSLC = np.zeros(len(lat))
                    BottomPressUSLC = np.zeros(len(lat))
                    ...
                    BottomTempIceCloud = np.zeros(len(lat))
                    BottomSHIceCloud = np.zeros(len(lat))
                    BottomPressIceCloud = np.zeros(len(lat))
                    ...
                    BottomTempWarmCloud = np.zeros(len(lat))
                    BottomSHWarmCloud = np.zeros(len(lat))
                    BottomPressWarmCloud = np.zeros(len(lat))
                    
                    CoupledClouds = np.zeros(len(lat))
                    DecoupledClouds = np.zeros(len(lat))
                    ...
                    def PropertiesMPC(nx,topcloud,chkcloud,remainCLEAR):
                        ...
                        #Information on the first non-cloudy pixel below cloud base
                        if topcloud+chkcloud+remainCLEAR < len(alti):
                            idx_BottomAlti = np.where(altiFull==alti[topcloud+chkcloud+remainCLEAR])[0][0]
                        else :
                            idx_BottomAlti = (np.where(altiFull==alti[topcloud+chkcloud+remainCLEAR-1])[0][0])+1

                        BottomTempMPC[nx] = temperatureFull[nx, idx_BottomAlti]
                        BottomSHMPC[nx] = spec_hum[nx, idx_BottomAlti]*1000
                        BottomPressMPC[nx] = pressure[nx, idx_BottomAlti]
                    ...
                    def PropertiesUSLC(nx,topcloud,chkcloud,remainCLEAR):
                        ...
                        BottomTempUSLC[nx] = temperatureFull[nx, idx_BottomAlti]
                        BottomSHUSLC[nx] = spec_hum[nx, idx_BottomAlti]*1000
                        BottomPressUSLC[nx] = pressure[nx, idx_BottomAlti]
                    ...
                    def PropertiesIceCloud(nx,topcloud,chkcloud,remainCLEAR):
                        ...
                        BottomTempIceCloud[nx] = temperatureFull[nx, idx_BottomAlti]
                        BottomSHIceCloud[nx] = spec_hum[nx, idx_BottomAlti]*1000
                        BottomPressIceCloud[nx] = pressure[nx, idx_BottomAlti]
                    ...
                    def PropertiesWarmCloud(nx,topcloud,chkcloud,remainCLEAR):
                        ...
                        BottomTempWarmCloud[nx] = temperatureFull[nx, idx_BottomAlti]
                        BottomSHWarmCloud[nx] = spec_hum[nx, idx_BottomAlti]*1000
                        BottomPressWarmCloud[nx] = pressure[nx, idx_BottomAlti]

                    
                    def Coupling(nx, BottomPressCloud, BottomTempCloud, BottomSHCloud):
                        coeffPot = (100000./surf_press[nx])**(287.04/1005.7)
                        SurfTempPot = tempe2m[nx] * coeffPot
                        
                        
                        gammaDP = (17.62*(tempe2m[nx]-273.15))/(243.12+(tempe2m[nx]-273.15))-0.163
                        DP = (243.12*gammaDP)/(17.62-gammaDP)
                        LCL = 125*((tempe2m[nx]-273.15)-DP)
                        idx_LCL = (np.abs(altiFull - LCL/1000)).argmin()
                       
                        coeffPotSurf = (100000./pressure[nx, idx_LCL])**(287.04/1005.7)
                        LCLTempPot = temperatureFull[nx, idx_LCL] * coeffPotSurf
                        
                        DiffTemp = LCLTempPot - SurfTempPot
                        
                        SH_LCL = spec_hum[nx,idx_LCL]*1000
                        DiffSH = SH_LCL - BottomSHCloud
                        
                        if DiffTemp <= 1. :
                            CoupledClouds[nx] = 1
                        else :
                            DecoupledClouds[nx] = 1
                    ...
                    for nx in range(len(lat)):
                        ...
                        if nansum(Allpix_in_cloud[nx,:]) >= 3 :
                            ...
                            for nz in range(len(alti)-1,-1,-1):
                                ...
                                BottomTemp = temperatureFull[nx, idx_BottomAlti]
                                BottomSH = spec_hum[nx, idx_BottomAlti]*1000
                                BottomPress = pressure[nx, idx_BottomAlti]
                                ...
                                if chkCLEAR == 3:
                                    ...
                                    if (chkcloud >= 3 and CoupledTest ==1 ) :
                                        TotalClouds[nx] += 1
                                        if TotalClouds[nx] == 1:
                                        ...
                                        if (chkSCWice+chkICE+chkSCW) >= 1:
                                            ...
                                            if chkSCWice >= 1:
                                                ...
                                                if MPC[nx] == 1:
                                                    ...
                                                    Coupling(nx, BottomPressMPC[nx], BottomTempMPC[nx], BottomSHMPC[nx])
                                            ...
                                            # Ice-only cloud
                                                ...
                                                if IceCloud[nx] == 1:
                                                    ...
                                                    Coupling(nx, BottomPressIceCloud[nx], BottomTempIceCloud[nx], BottomSHIceCloud[nx])
                                                if tempe[nx,topcloud] <= 273.15:
                                                    IceCloudTemp[nx] += 1
                                            ...
                                            # USLC
                                                ...
                                                if chkWARM == 0 :
                                                    ...
                                                    if USLC[nx] == 1:
                                                        ...
                                                        Coupling(nx, BottomPressUSLC[nx], BottomTempUSLC[nx], BottomSHUSLC[nx])
                                                    if tempe[nx,topcloud] <= 273.15:
                                                        USLCTemp[nx] += 1
                                        ...
                                        # The cloud is warm if it does not contain any cold pixel.
                                            ...
                                            if WarmCloud[nx] == 1:
                                                ...
                                                Coupling(nx, BottomPressWarmCloud[nx], BottomTempWarmCloud[nx], BottomSHWarmCloud[nx])
                                            
                                    # 3 consecutive precipitation pixels = 1 precipitation layer
                                    # Cumulative with clouds
                                    if (chkprecip >= 3 and CoupledTest == 1):
                                        Precip[nx] += 1
                                        if Precip[nx] == 1:
                                            PrecipMatrix[nx] = 1
                                    
                                    # Tests        
                                    if (chkcoldR >= 3 and CoupledTest == 1):
                                        ColdRain[nx] += 1
                                        if ColdRain[nx] == 1:
                                            ColdRainMatrix[nx] = 1
                                    if (chkColdPrecip >= 3 and CoupledTest == 1):
                                        ColdPrecip[nx] += 1
                                        if ColdPrecip[nx] == 1:
                                            ColdPrecipMatrix[nx] = 1
                                    if (chkWarmPrecip >= 3 and CoupledTest == 1):
                                        WarmPrecip[nx] += 1
                                        if WarmPrecip[nx] == 1:
                ...
                if CloudAnalysis == True :
                    ...
                    if coupling == True :
                        if not 'CoupledClouds_count' in locals():
                            CoupledClouds_count = array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                        if not 'DecoupledClouds_count' in locals():
                            DecoupledClouds_count = array(np.zeros(nbLat*nbLong).reshape((nbLat,nbLong)),float)
                ...
                for nlat in range(len(rangeLat)-1):
                    for nlong in range(len(rangeLong)-1):
                        ...
                        if idx_deltaLat != [] and idx_deltaLat2 != []:
                            ...
                            if coupling == True:
                                if not np.isnan(nansum(CoupledClouds[idx_deltaLat2])):
                                    CoupledClouds_count[nlat,nlong] += nansum(CoupledClouds[idx_deltaLat2])
                                if not np.isnan(nansum(DecoupledClouds[idx_deltaLat2])):
                                    DecoupledClouds_count[nlat,nlong] += nansum(DecoupledClouds[idx_deltaLat2])
                            ...
                            if coupling == True:
                                CoupledClouds_count[nlat,nlong] += 0.
                                DecoupledClouds_count[nlat,nlong] += 0.
        ...
        if cond1 + cond2 + cond3 > 0.:        
            ...
            if coupling == True :
                nw += 2
                if 'CoupledClouds_count' in locals(): writingOK += 1.
                if 'DecoupledClouds_count' in locals(): writingOK += 1.
            ...
            if goWriting==True and textFile==True:
                ...
                if coupling==True and textFile==True:
                    
                    fw = open(pfig+"/"+filesFolder+"/CoupledClouds_COVER_"+vers+"_"\
                        +str(day1.tm_year*10000+day1.tm_mon*100+day1.tm_mday)+"_to_"\
                        +str(day2.tm_year*10000+day2.tm_mon*100+day2.tm_mday)+".txt", "w")
                    fw.write("Sum of surface-coupled clouds : \n")
                    fw.write(enteteStats)
                    f2 = writer(fw, delimiter='\t', lineterminator='\n')
                    f2.writerows(CoupledClouds_count.T)
                    fw.close()
                    
                    fw = open(pfig+"/"+filesFolder+"/DecoupledClouds_COVER_"+vers+"_"\
                        +str(day1.tm_year*10000+day1.tm_mon*100+day1.tm_mday)+"_to_"\
                        +str(day2.tm_year*10000+day2.tm_mon*100+day2.tm_mday)+".txt", "w")
                    fw.write("Sum of surface-decoupled clouds : \n")
                    fw.write(enteteStats)
                    f2 = writer(fw, delimiter='\t', lineterminator='\n')
                    f2.writerows(DecoupledClouds_count.T)
                    fw.close()
                ...
                if CloudAnalysis == True:
                    del MPC_count, MPCTemp_count, MPCMatrix_count,\
                ...
                if coupling == True:
                    del CoupledClouds_count, DecoupledClouds_count
    ...
    # Ending prints
    print "Climato DARDAR V2 - CLOUD COVER STEREOGRAPHIC PROJECTION ON SOUTHTERN OCEAN (40-60S)"
    print "Parameters:"
    print "year(s):\t\t", lYears
    print "month(s):\t\t", m
    print "day(s):\t\t\t", d
...
segFile, altiMin, altiMax, instruFlag, landWaterFlag, dayNightFlag=None,\
...
CloudAnalysis=True, MeteoConditions=True, Winds=False, coupling=True, CloudSatPrecip=False, vers=versFig, textFile=True)
...
IceContaining_cloud_2_mask=False, liquidWarm_mask=False, classif_mix_clouds=False,\
iwc_val=False, ext_val=False, re_val=False, lidarRatio_val=False, temperature=True, temperature_2m=True,\
CloudAnalysis=True, MeteoConditions=True, Winds=True, CloudSatPrecip=False, vers=versFig, textFile=True)
"""